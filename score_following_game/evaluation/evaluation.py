...
class Evaluator:
    ...
    def _eval_pool(self, agent, pool, verbose):
        ...
        # get observations
        episode_reward = 0
        # - : Gym API: env.reset() 過去僅回傳 observation。
        # observation = env.reset()
        observation, _ = env.reset() # In Gymnasium, reset returns (obs, info)
        # + : Gymnasium API: env.reset() 現在回傳 (obs, info)，此處 info 未被使用。
        ...
        while True:
            ...
            # - : Gym API: env.step() 過去回傳 (observation, reward, done, info)。
            # observation, reward, done, info = env.step(action)
            observation, reward, terminated, truncated, info = env.step(action)
            # + : Gymnasium API: env.step() 現在回傳 (obs, reward, terminated, truncated, info)。
            done = terminated or truncated
            # + : Gymnasium API: 'done' 旗標現在是 'terminated' 或 'truncated' 的組合。
...
class PerformanceEvaluator(Evaluator):
    ...
    def __init__(self, make_env, evaluation_pools, config, trials=1, render_mode=None):
        ...
        super().__init__(make_env, evaluation_pools, config, trials, render_mode)
        # + : 改為使用 super() 呼叫父類別建構函式，此為 Python 3 繼承的建議作法。
...
class EmbeddingEvaluator(Evaluator):
    ...
    def _eval_pool(self, agent, pool, verbose):
        ...
        # - : Gym API: env.reset() 過去回傳值未被使用。
        # plain_env.reset()
        _, _ = plain_env.reset() # In Gymnasium, reset returns (obs, info)
        # + : Gymnasium API: env.reset() 現在回傳 (obs, info)，此處回傳值皆未被使用。
        ...
        while True:
            ...
            # - : Gym API: plain_env.step() 過去回傳 (obs_org, r, d, _info_plain)，其中僅 obs_org 被使用。
            # obs_org, r, d, _ = plain_env.step(action)
            obs_org, _, _, _, _ = plain_env.step(action) # Gymnasium returns (obs, reward, terminated, truncated, info)
            # + : Gymnasium API: env.step() 現在回傳 (obs, reward, terminated, truncated, info)，此處僅 obs_org 被使用。
            ...
            return_dicts['target_lost'].append(done) # 'done' 來自主要 env 的 step 結果
            ...
            if done: # 'done' 來自主要 env 的 step 結果
                break
